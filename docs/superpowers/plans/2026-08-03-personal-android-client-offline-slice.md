...
- Create: `client-app/src/screens/reports.css`
- Modify: `client-app/src/App.tsx`
- Modify: `client-app/src/components/FilterBar.tsx`
- Modify: `client-app/src/components/SharedComponents.test.tsx`
...
初始加载显示骨架；首次读取失败且没有内容时显示脱敏页面级失败；已有内容后的再次读取失败保留旧内容并显示 stale notice。报告库必须区分“筛选无结果”和“没有历史报告”，并用请求序号或取消标记防止旧筛选结果覆盖新筛选结果。
...
2026-08-12 验收记录：用户通过局域网临时二维码在型号代码 `V2408A`、Android 15 的自有手机安装同一 Debug APK，隔日重新打开正常；五张截图覆盖今日、报告库、报告详情、研究总览和“我的”，用户未发现阻断问题。未记录序列号或 IMEI。
...
2026-08-12 最终证据：客户端 Vitest 149/149、E2E 17/17、E2E 生命周期 2/2、报告引擎 124/124，客户端与报告引擎构建、lint、生产依赖审计和 Android `assembleDebug` 均通过；APK 身份、权限、密钥模式和嵌入 URL 重新扫描通过，真机人工冒烟见同阶段验证记录。
