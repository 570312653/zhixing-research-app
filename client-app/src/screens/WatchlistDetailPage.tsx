...
import './research.css'
import { DisabledResearchRefresh, ResearchDetailState, ResearchFailure, ResearchLoading, ResearchStale } from './researchShared'
...
export function WatchlistDetailPage({ repository = defaultResearchRepository }: { repository?: ReportRepository }) {
  ...
  if (state.kind === 'loading') return <ResearchDetailState title="标的详情" backTo="/research/watchlist" backLabel="返回标的池"><ResearchLoading label="标的详情" /></ResearchDetailState>
  if (state.kind === 'failure') return <ResearchDetailState title="标的详情" backTo="/research/watchlist" backLabel="返回标的池"><ResearchFailure /></ResearchDetailState>
  if (state.kind === 'missing') return <ResearchDetailState title="标的详情" backTo="/research/watchlist" backLabel="返回标的池"><p className="research-empty">标的不存在或已不可用</p></ResearchDetailState>
  ...
  const visibleEvents = expandedHistorySymbol === item.symbol ? item.events.toReversed() : item.events.toReversed().slice(0, 5)
  const returnTo = `/research/watchlist/${item.symbol}` as const

  return <article className="research-page research-detail">
    <header className="research-detail__toolbar"><Link aria-label="返回标的池" to="/research/watchlist">‹</Link><div><small>标的详情</small><h1>{item.displayName}</h1></div><DisabledResearchRefresh /></header>
    ...
    <section className="research-section" role="region" aria-label="观察历史"><h2>观察历史</h2><ol className="research-timeline">{visibleEvents.map((event) => <li key={event.id}><time>{formatTimestamp(event.occurredAt)}</time><strong>{eventLabels[event.type]}</strong><p>{event.reason}</p></li>)}</ol>{item.events.length > 5 && expandedHistorySymbol !== item.symbol && <button type="button" onClick={() => setExpandedHistorySymbol(item.symbol)}>展开全部观察历史</button>}</section>
