...
export function WatchlistPage({ repository = defaultResearchRepository }: { repository?: ReportRepository }) {
  ...
  return <section className="research-page">
    <header className="research-page__header"><div><p>研究观察清单，不构成投资建议</p><h1>核心关注标的</h1></div><span>快照时间：{formatTimestamp(overview.snapshotAt)}</span><button type="button" disabled title="离线固定样例未接入刷新">刷新</button></header>
    <ResearchTabs />
    {state.kind === 'stale' && <ResearchStale lastSuccessfulSyncAt={overview.snapshotAt} />}
    <div className="research-metrics"><strong>当前关注 {overview.currentItems.length}</strong><strong>本期新增 {overview.delta.added.length}</strong><strong>原因更新 {overview.delta.reasonChanged.length}</strong><strong>近期移出 {overview.delta.removed.length}</strong></div>
    <div className="research-switch"><button className={tab === 'current' ? 'is-active' : ''} type="button" onClick={() => setTab('current')}>当前关注</button><button className={tab === 'changes' ? 'is-active' : ''} type="button" onClick={() => setTab('changes')}>变更记录</button></div>
    {tab === 'current' ? <>
      ...
      {visible.length === 0 && <p className="research-empty">当前筛选条件下没有标的</p>}
      <p className="research-empty">移出标的不在当前列表展示，可切换“变更记录”追溯。</p>
    </> : <ul className="research-list">{overview.changes.map((change) => <ChangeItem key={`${change.type}-${change.symbol}`} change={change} />)}</ul>}
  </section>
}
...
function CurrentItem({ item, changeType, industryName }: { item: WatchlistOverviewItem; changeType: 'added' | 'reason_changed' | 'continuing'; industryName?: string }) {
  ...
  return <li>
    <div className="research-card__heading"><Link aria-label={`查看标的：${item.displayName}`} to={`/research/watchlist/${item.symbol}`}><strong>{item.displayName}</strong> <span>{item.symbol}</span></Link><span>{statusLabel}</span></div>
    ...
    <small>首次 {formatTimestamp(item.firstObservedAt)} · 最近 {formatTimestamp(item.lastObservedAt)}</small>
  </li>
}
