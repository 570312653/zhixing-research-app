...
export function IndustryListPage({ repository = defaultResearchRepository }: { repository?: ReportRepository }) {
  ...
  return <section className="research-page">
    ...
    <div className="research-filters">
      ...
      <div role="group" aria-label="行业趋势筛选">{filters.map((filter) => <button aria-pressed={trend === filter} className={trend === filter ? 'is-active' : ''} type="button" key={filter} onClick={() => setTrend(filter)}>{filter === 'all' ? '全部' : trendLabels[filter]}</button>)}</div>
    ...
    {data.industries.length === 0 ? <ContextualEmptyState reason="no_industries" /> : visible.length === 0 && <ContextualEmptyState reason="filter_no_industries" />}
  </section>
}
